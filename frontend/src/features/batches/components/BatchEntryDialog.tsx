...
type Props = { sites: ProductionSite[]; productConfigs: ProductConfig[]; onCreateBatch: (values: CreateBatchInput) => void };
...
export function BatchEntryDialog({ onCreateBatch, productConfigs, sites }: Readonly<Props>) {
  ...
  function applyExtraction(extraction: BatchTextExtraction) {
    ...
    const matchingConfig = productConfigs.find((candidate) => candidate.productionSiteId === matchedSite?.id && sameText(candidate.species, fields.species) && sameText(candidate.productSpec, fields.productSpecification));
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onCreateBatch({ productionSiteId: site.id, productionLineIds: lineIds, species: selectedConfig.species, productSpec: selectedConfig.productSpec, shift, supplier: supplier.trim() || undefined, fishSizeCategory: sizeCategory || undefined, deliveryDelayMinutes: numberOrUndefined(deliveryDelay), rejectReason: rejectReason.trim() || undefined, notes: notes.trim() || undefined, rawInputKg: inputKg, sellableOutputKg: outputs.sellable, normalByproductKg: outputs.byproduct, trimmingKg: outputs.trimming, qualityRejectKg: outputs.reject, spoilageKg: outputs.spoilage, otherLossKg: outputs.other });
    close();
  }
  ...
  return <Dialog onOpenChange={(nextOpen) => { if (!nextOpen) reset(); setOpen(nextOpen); }} open={open}>
    ...
    <DialogContent className="max-h-[calc(100dvh-4rem)] max-w-[56rem] gap-0 overflow-y-auto rounded-none border-[var(--line-strong)] bg-[var(--surface)] p-0 shadow-[0_24px_64px_-32px_rgb(19_56_82_/_55%)]">
      ...
      {step === "describe" ? <div className="space-y-5 px-6 py-6">
        <div className="border border-[var(--line)] bg-[var(--surface-subtle)] px-4 py-3 text-sm leading-6 text-[var(--muted)]"><Sparkles aria-hidden="true" className="mr-2 inline text-[var(--brand)]" size={16} />Example: <span className="text-[var(--ink)]"> tuna fillet beku, bahan baku 100 kg, hasil jual 70 kg, trimming 10 kg, reject 5 kg, produk samping 10 kg, spoilage 3 kg, kehilangan lain 2 kg</span></div>
        ...
        <DialogFooter className="gap-3 px-6 py-4 sm:justify-between sm:space-x-0"><p className="max-w-md text-xs leading-5 text-[var(--muted)]">Values are saved as a draft and remain outside comparable history until confirmation.</p><div className="flex gap-3"><Button className="rounded-none" onClick={() => setStep("describe")} type="button" variant="outline"><ArrowLeft aria-hidden="true" size={15} /> Edit description</Button><Button className="rounded-none bg-[var(--brand)] text-white hover:bg-[var(--brand-strong)] disabled:opacity-45" disabled={!site || !selectedConfig || lineIds.length === 0 || !shift || inputKg === undefined} type="submit">Save as draft</Button></div></DialogFooter>
      </form>}
    </DialogContent>
  </Dialog>;
}
