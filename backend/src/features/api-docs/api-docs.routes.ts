...
const openapi = {
  ...
  components: {
    ...
    schemas: {
      ...
      WhatsAppIdentityInput: { type: "object", required: ["phoneNumber"], properties: { phoneNumber: { type: "string", description: "The user's WhatsApp number in E.164 digits, without the plus sign.", example: "6281234567890" } } },
      DashboardConversationInput: { type: "object", properties: { restart: { type: "boolean", default: false, description: "Closes the active conversation and starts a new one." } } },
      WhatsAppDashboardMessageInput: { type: "object", required: ["text"], properties: { text: { type: "string", minLength: 1, maxLength: 4000, example: "tambah batch" } } },
  ...
  paths: {
    ...
    "/v1/whatsapp/identity": {
      get: { ...secured("Read the authenticated user's linked WhatsApp number"), responses: { "200": { description: "Linked identity, or null when none is linked" } } },
      put: { ...secured("Link the authenticated development user's WhatsApp number"), description: "Development-only manual link. A future flow must verify phone-number ownership before enabling self-service linking.", requestBody: jsonBody({ $ref: "#/components/schemas/WhatsAppIdentityInput" }), responses: { "200": { description: "WhatsApp identity linked" }, "409": { description: "Number is already linked to another LAUT user" } } }
    },
    "/v1/whatsapp/conversations": {
      get: { ...secured("List the authenticated user's WhatsApp conversations"), description: "Each conversation is scoped through the verified WhatsApp identity belonging to the authenticated profile." },
      post: { ...secured("Start or resume the active dashboard conversation"), description: "Requires a verified WhatsApp identity but does not deliver a message through Vonage. Set restart to true to close the active conversation and begin a new one.", requestBody: jsonBody({ $ref: "#/components/schemas/DashboardConversationInput" }), responses: { "201": { description: "Active conversation" }, "422": { description: "No verified WhatsApp identity is linked" } } }
    },
    "/v1/whatsapp/conversations/{conversationId}/messages": {
      get: { ...secured("Read messages for one owned WhatsApp conversation"), parameters: [pathId("conversationId")], responses: { "200": { description: "Message history" }, "404": { description: "Conversation is not owned by the authenticated user or does not exist" } } },
      post: { ...secured("Send a dashboard message through the batch wizard"), description: "Records a web transcript message and runs the same deterministic wizard as WhatsApp without relaying it to the phone.", parameters: [pathId("conversationId")], requestBody: jsonBody({ $ref: "#/components/schemas/WhatsAppDashboardMessageInput" }), responses: { "201": { description: "Message and assistant reply recorded" }, "409": { description: "Conversation is not active" } } }
    },
