...
const openapi = {
  ...
  paths: {
    ...
    "/v1/whatsapp/conversations": { get: { ...secured("List the authenticated user's WhatsApp conversations"), description: "Each conversation is scoped through the verified WhatsApp identity belonging to the authenticated profile." } },
    "/v1/whatsapp/conversations/{conversationId}/messages": { get: { ...secured("Read messages for one owned WhatsApp conversation"), parameters: [pathId("conversationId")], responses: { "200": { description: "Message history" }, "404": { description: "Conversation is not owned by the authenticated user or does not exist" } } } },
